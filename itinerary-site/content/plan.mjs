...
export const plan = {
  ...
  // No `altStays` / `levers` here any more. The one swap still open — Tobira Myojinkan against
  // Beniya Mukayu — is an open question rather than a feature of the plan, so it lives in
  // content/issues.mjs as `alps-or-coast` and renders Beniya Mukayu's card there. The archived
  // alternates in content/itineraries.mjs still carry their own; those are frozen.
